...
\begin{figure}[!htb]
% [!htb] [!Here Top Botton {Picture}] LaTeX escolhe melhor posicao entre as tres opcoes. Tambem pode ser usada apenas uma opcao. {P:Picture} mantem todas as figuras e graficos em uma mesma pagina
\center\includegraphics{./pic/C410processo.eps}
...
A Figura \ref{fig:malhaAberta} representa uma expansão do diagrama, subdividindo-o em blocos menores, mais específicos, e neste caso um sistema de controle em malha aberta, ou seja, não há uma reinserção do sinal de saída à entrada, chamada de realimentação ou retroalimentação. Assim a entrada possui o valor de resposta desejada, que alimenta o processo e a saída apresenta a resposta real, porém nada garante que a resposta real está coerente ao valor de entrada. 

\begin{figure}[!htb]
\center\includegraphics{./pic/C412malhaAberta.eps}
\caption{Diagrama em bloco de sistema de controle em malha aberta}
...
\begin{figure}[!htb]
\center\includegraphics{./pic/C414malhaFechada.eps}
\caption{Diagrama em bloco de sistema de controle em malha fechada}
...
%A otimização de parêmtros e análise de performance pressupoe uma avaliacao quanto ao desempenho do modelo em comparação ao comportamento real do sistema, devendo inclusive retornar e rever o diagrama de blocos, modelos matemáticos, descrição do controlador e parâmetros de ajuste e novamente realizar uma nova análise.

%\subsection{Método de sintonia - Ziegler-Nichols}

\end{enumerate}


O presente trabalho explicita a construção de um sistema de controle para um processo térmico de aquecimento no capítulo de desenvolvimento.
